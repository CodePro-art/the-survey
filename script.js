...
function slideShow() {
    ...
    slides.forEach(function (slide) {
        let links = slide.querySelectorAll(".next-slide-btn");
        links.forEach(function (link) {
            link.addEventListener("click", function (e) {
                e.preventDefault();
                
                slideNum = dataSlideToIndex(link.getAttribute("data-slide"));
                if (slideNum == "1" && login_name.toLowerCase() === "mr. timabundinn") 
                    slideNum = 9;
...
const timButton = document.getElementById('timButton');
const displayName = document.getElementById('displayName');
const nameRegex = /^[A-Za-z.-]+ [A-Za-z.-]+$/;

// Function to validate full name and show the submit button if valid
...
// Listen for the 'input' event to validate on every keystroke
